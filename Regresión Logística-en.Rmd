...
La tokenización se realiza en pasos:
...
Cuando el modelo predice de manera incorrecta el valor de la función `loss` es cercano a `9.2`.

Por ejemplo, si predice cercano a `1` y el valor real es 0:

```{r}
-1 * sum(0 * log(0.9999) + (1-0) * log(1-0.9999))
```

O bien, cuando predice cercano a cero y el valor real es 1:
